...
void ArdupilotGuided::offboard_loop_callback()
{
    ...
    std::shared_lock<std::shared_mutex> lock(node_data_mutex_); // Use shared_lock for data reads
    // if (!((aircraft_fsm_state_ == ArdupilotGuidedState::OFFBOARD_VELOCITY) || (aircraft_fsm_state_ == ArdupilotGuidedState::OFFBOARD_ACCELERATION))) {
    ...
    // if (aircraft_fsm_state_ == ArdupilotGuidedState::OFFBOARD_VELOCITY) {
    ...
    // }
    // else if (aircraft_fsm_state_ == ArdupilotGuidedState::OFFBOARD_ACCELERATION) {
